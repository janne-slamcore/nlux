(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global["@nlux/openai"] = {}));
    ...
    const conversationHistoryToMessagesList = (conversationHistory) => {
      return conversationHistory.map((item) => {
        ...
        if (content === void 0) {
          warn(
            `Empty message or unsupported message format found in conversation history and will not be included in the conversation history sent to OpenAI.`
          );
          return void 0;
        }
        return {
...
`;
    ...
    const gptAdapterInfo = Object.freeze(info);

    const defaultChatGptModel = "gpt-4-turbo";
    const defaultDataTransferMode = "stream";

    var __defProp$1 = Object.defineProperty;
    var __defNormalProp$1 = (obj, key, value) => key in obj ? __defProp$1(obj, key, { enumerable: true, configurable: true, writable: true, value }) : obj[key] = value;
    var __publicField$1 = (obj, key, value) => __defNormalProp$1(obj, typeof key !== "symbol" ? key + "" : key, value);
    class OpenAiAbstractAdapter {
      constructor({
        systemMessage,
        apiKey,
        dataTransferMode,
        ...
        __publicField$1(this, "openai");
        __publicField$1(this, "systemMessage", "Act as a helpful assistant to the user");
        __publicField$1(this, "theDataTransferMode");
        ...
        warn(
          `OpenAI GPT adapter has been initialized in browser mode using option "dangerouslyAllowBrowser". To learn more about OpenAI' recommendation for handling API keys, please visit:
https://help.openai.com/en/articles/5112595-best-practices-for-api-key-safety
...
https://docs.nlkit.com/nlux/learn/adapters/custom-adapters`
    ...
    class OpenAiBatchAdapter extends OpenAiAbstractAdapter {
      constructor({
        apiKey,
        model,
        systemMessage
      ...
      async batchText(message, extras) {
        ...
        if (extras.conversationHistory) {
          const itemsInOpenAiFormat = conversationHistoryToMessagesList(extras.conversationHistory);
        ...
        try {
          ...
          throw new NluxUsageError({
            ...
            message: message2 ?? "Error while making API call to OpenAI",
    ...
    class OpenAiStreamingAdapter extends OpenAiAbstractAdapter {
      constructor({
        apiKey,
        model,
        systemMessage
      ...
      streamText(message, observer, extras) {
        ...
        if (extras.conversationHistory) {
          const newItems = conversationHistoryToMessagesList(
        ...
        this.openai.chat.completions.create({
          ...
          while (!result.done) {
            ...
            const finishReason = value.choices?.length > 0 ? value.choices[0].finish_reason : void 0;
    ...
    class OpenAiAdapterBuilderImpl {
      constructor(cloneFrom) {
        __publicField(this, "apiKey", null);
        __publicField(this, "dataTransferMode", defaultDataTransferMode);
        __publicField(this, "model", null);
        __publicField(this, "systemMessage", null);
        __publicField(this, "withApiKeyCalled", false);
        __publicField(this, "withDataTransferModeCalled", false);
        __publicField(this, "withModelCalled", false);
        __publicField(this, "withSystemMessageCalled", false);
        if (cloneFrom) {
      ...
      create() {
        if (!this.apiKey) {
          throw new NluxUsageError({
            source: this.constructor.name,
            message: "Unable to create OpenAI adapter. API key is missing. Make sure you are calling withApiKey() before calling create()."
          });
        }
        const options = {
          apiKey: this.apiKey,
          ...
          systemMessage: this.systemMessage ?? void 0
        };
      ...
      withApiKey(apiKey) {
        if (this.withApiKeyCalled) {
          throw new NluxUsageError({
            source: this.constructor.name,
            message: "Unable to set API key. API key setter has already been called by this builder. Make sure you are not calling withApiKey() twice."
          });
        }
        this.apiKey = apiKey;
      ...
      withDataTransferMode(mode) {
        if (this.withDataTransferModeCalled) {
          throw new NluxUsageError({
            source: this.constructor.name,
            message: "Unable to set data loading mode. Stream or fetch setter has already been called by this builder. Make sure you are not calling stream() or fetch() twice."
          });
        }
      ...
      withModel(model) {
        if (this.withModelCalled) {
          throw new NluxUsageError({
            source: this.constructor.name,
            message: "Unable to set model. Model setter has already been called by this builder. Make sure you are not calling withModel() twice."
          });
        }
        this.model = model;
      ...
      withSystemMessage(message) {
        if (this.withSystemMessageCalled) {
          throw new NluxUsageError({
            source: this.constructor.name,
            message: "Unable to set initial system message. Initial system message setter has already been called by this builder. Make sure you are not calling withSystemMessage() twice."
          });
        }
